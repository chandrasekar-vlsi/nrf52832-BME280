...
#define APP_BEACON_INFO_LENGTH          0x05                              /**< Total length of information advertised by the Beacon. */
...
static uint64_t m_beacon_info[1] = { 0x44332211 };
...
static void advertising_start(void)
{
    ...
    //err_code = sd_ble_gap_adv_stop(&m_adv_params);
    //APP_ERROR_CHECK(err_code);
}
...
void bme280_handler(void)
{
  ...
  NRF_LOG_INFO("raw_t %x\r\n", raw_t); // divide result by 100
  NRF_LOG_INFO("raw_p %x\r\n", raw_p); // divide result by 256
  NRF_LOG_INFO("raw_h %x\r\n", raw_h); // divide result by 1024
  output = (raw_p<<32) | (raw_h<<16) | raw_t ;
  NRF_LOG_INFO("output %x\r\n", output);
  m_beacon_info[0] = output;
  advertising_init();
}
